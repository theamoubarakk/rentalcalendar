...
with left_col:
    ...
    def get_booking_status(date):
        ...
        # join with real newlines so the tooltip shows separate lines
        tooltip_text = "\n".join(tooltip_parts)
        # Display shows ❌ + list of mascots
    ...
    for idx, day in enumerate(days):
        header_cols[idx].markdown(f"<div style='text-align:center;font-weight:bold;'>{day}</div>", unsafe_allow_html=True)
    ...
    for week in calendar.monthcalendar(month_filter.year, month_filter.month):
        ...
        for idx, day_num in enumerate(week):
            if day_num == 0:
                continue
            the_date = datetime(month_filter.year, month_filter.month, day_num)
            status_text, tooltip = calendar_df.loc[calendar_df["Date"] == the_date, "StatusTuple"].iloc[0]
            bg = "#f9e5e5" if status_text.startswith("❌") else "#e6ffea"
            icon, *rest = status_text.split(" ", 1)
            with row_cols[idx]:
                # raw HTML cell with title=tooltip
                st.markdown(
                    f"""
                    <div title="{tooltip}"
                         style="
                      ...
                      <span style="font-size:0.9em;">{icon} {rest[0] if rest else ''}</span>
                    </div>
                    """,
...
# ==============================================================================
# RIGHT COLUMN: (unchanged) Delete / Download / New Entry Form
# ==============================================================================
with right_col:
    st.markdown("### Delete or Download Bookings")
    # … keep your existing delete & download logic here …
